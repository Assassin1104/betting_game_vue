        ...
        methods: {
            getTable(val){
                ...
                window.axios.post(`${process.env.VUE_APP_URL}getPredictions`, [val]).then(({data})=> {
                    ...
                    for(let i = 0 ; i < data.data[0].length ; i++){
                        console.log('---->', data.data[0][i].label)
            ...
            getSearchResult(){
                ...
                if(this.searchFilter.duplicate === false){
                    let predictionList = this.eventLists2
                    this.predictionlist = [{'value': 1, 'label': 'Select Prediction','marketType': '', 'week': ''}]
                    let duplicatedPredictions = []
                    for(let i = 0 ;  i < predictionList.length ; i++){
                        let event_prediction = []
                        let eventStr1 = predictionList[i].value
                        let eventId1 = eventStr1.split(',')[2]
                        for(let j = 1 ; j < predictionList.length ; j++){
                            let eventStr2 = predictionList[j].value
                            let eventId2 = eventStr2.split(',')[2]
                            if(eventId1 === eventId2){
                                event_prediction.push({'value': predictionList[j].value, 'marketType': predictionList[j].marketType})
                            }
                        }
                        ...
                        if(event_prediction.length > 1){
                            for(let k = 0 ; k < event_prediction.length ; k++){
                                if(event_prediction[k].marketType === '- X - Back'){
                                    ...
                                    if(check === 1){
                                        for(let kk = 0 ; kk < event_prediction.length ; kk++){
                                            duplicatedPredictions.push(event_prediction[kk].value)
                                        }
                                    }
                                }
                    ...
                    this.duplicatedPredictions = duplicatedPredictions
                    console.log('duplicatedPredictions===>', this.duplicatedPredictions)
                }

                if(this.searchFilter.doubleTip === false){
                    let predictionList = this.eventLists2
                    let doubleTipPredictions = []
                            ...
                            for(let k = 0 ; k < event_prediction.length ; k++){
                                if(event_prediction[k].marketType === '- 1 - Lay'){
                                    ...
                                    if(check === 1){
                                        for(let kk = 0 ; kk < event_prediction.length ; kk++){
                                            if(event_prediction[kk].marketType === '- X - Lay'){
                                                doubleTipPredictions.push(event_prediction[k].value)
                                            }
                                        }
                                    }
                                }
                                    ...
                                    if(check === 1){
                                        for(let kk = 0 ; kk < event_prediction.length ; kk++){
                                            if(event_prediction[kk].marketType === '- 2 - Lay') {
                                                doubleTipPredictions.push(event_prediction[kk].value)
                                            }
                                        }
                                    }
                                }
                    ...
                    doubleTipPredictions = Array.from(new Set (doubleTipPredictions))
                    this.doubleTipPredictions = doubleTipPredictions
                }
                ...
                if(this.searchFilter.duplicate === true){
                    ...
                    for(let i = 1 ;  i < predictionList.length ; i++){
                        ...
                        for(let j = 1 ; j < predictionList.length ; j++){
                            ...
                            let eventId2 = eventStr2.split(',')[2]
                            if(eventId1 === eventId2){
                                event_prediction.push({'value': predictionList[j].value, 'marketType': predictionList[j].marketType, 'label': predictionList[j].label})
                            }
                        }
                            ...
                            for(let k = 0 ; k < event_prediction.length ; k++){
                                if(event_prediction[k].marketType === '- X - Back'){
                                    ...
                                    if(check === 1){
                                        for(let kk = 0 ; kk < event_prediction.length ; kk++){
                                            duplicatedPredictions.push(event_prediction[kk].value)
                                            console.log('event_prediction===>', event_prediction)
                                        }
                    ...
                    for(let i = 0 ; i < duplicatedPredictions.length ; i++){
                        let prediction = this.eventLists2.filter(function(item) {
                            return item.value === duplicatedPredictions[i];
                        });
                        for(let pp = 0 ; pp < prediction.length ;pp++){
                            this.predictionlist.push({'league':prediction[pp].league, 'pre_MatchOdd': prediction[pp].pre_MatchOdd,'predictionType': prediction[pp].predictionType,'value': prediction[pp].value, 'week': prediction[pp].week, 'label': prediction[pp].label, 'marketType': prediction[pp].marketType, 'scores': prediction[pp].scores, 'Odds':prediction[pp].Odds})
                        }
                    }
                    console.log('kingTest====>', duplicatedPredictions)
                }
                else{
                    let predictionList = this.predictionlist
                    let duplicatedPredictions = []
                    ...
                    for(let i = 1 ; i < this.predictionlist.length ; i++){
                        for(let j = 0 ; j < this.duplicatedPredictions.length ; j++){
                            let self = this
                            if(this.predictionlist[i].value === this.duplicatedPredictions[j]){
                                let prediction = this.predictionlist.filter(function(item) {
                                    return item.value === self.duplicatedPredictions[j];
                                });
                                console.log('duplicated events==>', prediction[0].label)
                                let index = this.predictionlist.indexOf(prediction[0]);
                                if (index > -1) {
                                    this.predictionlist.splice(index, 1);
                                    i = i - 1
                ...
                if(this.searchFilter.doubleTip === true){
                            ...
                            for(let k = 0 ; k < event_prediction.length ; k++){
                                if(event_prediction[k].marketType === '- 1 - Lay'){
                                    ...
                                    if(check === 1){
                                        ...
                                        duplicatedPredictions.push(event_prediction[k].value)
                                    }
                                }
                                else if(event_prediction[k].marketType === '- X - Lay'){
                                    let check = 0
                    ...
                    for(let i = 0 ; i < duplicatedPredictions.length ; i++){
                        ...
                        this.predictionlist.push({'league':prediction[0].league, 'pre_MatchOdd': prediction[0].pre_MatchOdd,'predictionType': prediction[0].predictionType,'value': prediction[0].value, 'week': prediction[0].week, 'label': prediction[0].label, 'marketType': prediction[0].marketType, 'scores': prediction[0].scores, 'Odds': prediction[0].Odds})
                    }
                }
                else{
                    for(let i = 0 ; i < this.doubleTipPredictions.length ; i++){
                        let self = this
                        let prediction = this.predictionlist.filter(function(item) {
                            return item.value === self.doubleTipPredictions[i];
                        });

                        for(let pp = 0 ; pp < prediction.length; pp++){
                            let index = this.predictionlist.indexOf(prediction[pp]);
                            if (index > -1) {
                                this.predictionlist.splice(index, 1);
                                i = i - 1
                            }
